...
func (s *RdsServiceImpl) pageQuery(query Trend, pageIndex, pageSize int) error {
	...
	if rst := s.db.Where(query).Offset(pageIndex * pageSize).Limit(pageSize).Find(&result.Data).Error; rst == nil {
		return rst
	}

	return s.db.Where(query).Count(&count).Error
}
